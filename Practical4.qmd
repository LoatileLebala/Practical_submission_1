...
Displaying the flights dataset in an alternative format. Used the glimpse and as_tibble functions to do this.
...
flight_F9 <- flights |> filter(carrier == "F9") 
flight_F9$distance #we see that the value for all the carrier's distances remain the same therefore if there is no variability around the distance values the standard deviation will be 0 as a result the same applies for below. 


flight_AS <- flights |> filter(carrier == "AS")
flight_AS$distance
...
delayed_arrival_prop <- flights %>%
  filter(dep_delay > 0 & arr_delay <= 0) %>%
  summarise(proportion = n() / nrow(flights))

print(delayed_arrival_prop)
...
Identify the routes that more than one airline flies
...
Calculate the average arrival delay for each airline, exlcuding NAs
...
Identify the airline with the worst and best average arrival delay
...
UtilsDataRSV::view_cols(data,n_entry = 50)
